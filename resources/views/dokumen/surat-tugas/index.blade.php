<x-app-layout>
    <x-slot name="header">
        ...
        <p class="text-sm text-gray-500 mt-1">Cetak surat tugas otomatis berdasarkan penugasan liputan.</p>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl border border-gray-100">
                <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
                    <h3 class="text-lg font-bold text-gray-900">Daftar Penugasan untuk Surat Tugas</h3>
                    <p class="text-sm text-gray-500 mt-1">Klik "Cetak Surat Tugas" untuk mengunduh surat otomatis sesuai data aplikasi.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($penugasan as $p)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-gray-900">{{ $p->kegiatan->judul_kegiatan }}</div>
                                        <div class="text-xs text-gray-500">{{ $p->kegiatan->kategori->nama_kategori ?? 'Umum' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($p->kegiatan->tanggal)->format('d M Y') }}</td>
                                    <td class="px-6 py-4">{{ $p->kegiatan->lokasi }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <a href="{{ route('dokumen.surat-tugas.cetak', $p->id) }}" target="_blank"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-gray-500">Belum ada penugasan yang dapat dibuatkan surat tugas.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
