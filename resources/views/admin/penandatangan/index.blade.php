                    ...
                    <table class="w-full text-left text-sm text-gray-600">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($penandatangan as $item)
                                <tr class="hover:bg-gray-50/50 transition-colors duration-200" x-data="{ openEdit: false, openDelete: false, openPreview: false }">
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <button @click="openPreview = true" class="p-2 text-emerald-600 bg-emerald-50 rounded-lg hover:bg-emerald-100 hover:text-emerald-800 transition-colors" title="Preview">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                            </button>
                                            <button @click="openEdit = true" class="p-2 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 hover:text-blue-800 transition-colors" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </button>
                                            <button @click="openDelete = true" class="p-2 text-red-600 bg-red-50 rounded-lg hover:bg-red-100 hover:text-red-800 transition-colors" title="Hapus">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                            </button>
                                        </div>

                                        <div x-show="openPreview" style="display: none;" class="relative z-50">
                                            <div class="fixed inset-0 bg-gray-900 bg-opacity-75"></div>
                                            <div class="fixed inset-0 z-10 overflow-y-auto">
                                                <div class="flex min-h-full items-center justify-center p-4 text-center">
                                                    <div class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-2xl transition-all sm:w-full sm:max-w-md" @click.away="openPreview = false">
                                                            <div class="p-6">
                                                                <h3 class="text-lg font-bold text-gray-900 mb-4">Preview Penandatangan</h3>
                                                                <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                                                                    <div class="text-center mb-4">
                                                                        @if($item->qr_code_path)
                                                                            <img src="{{ asset($item->qr_code_path) }}" alt="QR Code" class="inline-block w-24 h-24 border-2 border-gray-200 rounded-lg">
                                                                        @else
                                                                            <div class="inline-flex items-center justify-center w-24 h-24 bg-gray-200 rounded-lg text-gray-400">
                                                                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                                                        </div>
                                                                        @endif
                                                                    </div>
                                                                    <div class="space-y-2 text-sm">
                                                                        <div class="flex justify-between"><span class="text-gray-500">Nama:</span><span class="font-bold text-gray-900">{{ $item->nama_pejabat }}</span></div>
                                                                        <div class="flex justify-between"><span class="text-gray-500">NIP:</span><span class="font-mono text-gray-900">{{ $item->nip }}</span></div>
                                                                        <div class="flex justify-between"><span class="text-gray-500">Jabatan:</span><span class="text-gray-900">{{ $item->jabatan }}</span></div>
                                                            ...
                                                            <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 rounded-b-xl">
                                                                <button type="button" @click="openPreview = false" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Tutup</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
